...
- 메인 페이지는 큰 키워드 검색창과 추천 주제 칩만 보이는 검색 전용 화면
...
- 비회원은 가입 없이 키워드 게임 전체와 기본 MBTI 캐릭터 결과 이용
- 회원은 결정적 선택·선택 사용설명서·4축 나침반 상세 결과 이용
- 회원 결과는 `SavedInstantResult`에 자동 저장되며 같은 게임을 수정하면 중복 없이 갱신
...
- 비회원 공통: 선택한 답변과 문항별 성향 코드를 근거로 도출한 4자리 MBTI 캐릭터
- 숨은 본성을 요약한 코믹 타이틀과 3문장 이내의 분석
- 회원 전용: 결과에 영향을 준 실제 답변 3개를 보여주는 `결정적 선택`
