...
메인 화면에서 궁금한 키워드를 입력하면 7개의 맞춤 양자택일이 생성되고 바로 플레이할 수 있는 선택형 웹서비스입니다. 준비된 공식 콘텐츠도 주제별로 플레이할 수 있으며, 완료 후 **답변을 근거로 한 MBTI 캐릭터·코믹 해석·선택 패턴 분석**을 확인할 수 있습니다.
...
| 결과 | 투표 비율만 표시 | **문항별 7단계 등급 + 답변 기반 MBTI 캐릭터·코믹/패턴 분석** |
...
- 7문항 완료 후 답변을 근거로 한 MBTI 캐릭터·코믹 해석·선택 패턴 분석 제공
...
│   │   ├── instant_result.html  # 즉석 게임 유형·패턴 결과
│   │   ├── list.html            # 게임 목록
...
| `/instant-game/result/` | `InstantGameResultView` | 즉석 게임 유형·패턴 결과 |
...
- 16개 MBTI 코드 중 하나, 코믹 타이틀, 3문장 이내 설명으로 응답 스키마를 제한
