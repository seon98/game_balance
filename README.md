...
| 사용자 기능 | 없음 | **회원가입·로그인 + 키워드 자동 초안 + 7~10문항 게임 제작** |
...
회원이 카테고리와 키워드 1~5개, 문항 수 7~10개를 입력하면 A/B 질문 초안을 자동으로 채웁니다.

- 별도 API 키 없이 즉시 작동하는 가상 상황 초안 생성
- 제작 화면 중앙에 카테고리·키워드·문항 수 입력 영역 배치
- 공통 16개 관점과 카테고리별 21개 상황을 조합해 반복 생성 시 다른 문항 제공
- 키워드가 여러 개면 키워드끼리 직접 비교하는 문항도 포함
- 생성 단계와 최종 제출 단계에서 안전 필터를 각각 적용
- 성인·유해 키워드와 검증이 필요한 단정 표현은 생성 전에 차단
- 자동 저장·공개하지 않고 제작자가 수정한 뒤 관리자 검수로 제출
...
│   ├── question_generator.py    # 키워드 기반 안전한 질문 초안 생성
